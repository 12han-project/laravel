(function($) {
    $.fn.upload = function(look_call_back){
                    ...
                    var rs = addfile(fileObj[item],function(data,item,file){
                        ...
                        $.ajax({
                            ...
                            success:function(data){
                                ...
                                {
                                    item.addClass("error").attr("data-error",data.msg?data.msg:"error。4012");
                                }
                                setfilelist();
                            },
                            xhr:function(){
                            ...
                            error:function(){
                                item.addClass("error").attr("data-error","ネット異常！");
                                setfilelist();
                            }
                        });
                    });
        ...
        function addfile(file,callback)
        {
            ...
            {
                var errorstr = "."+exp+"のファイルをアップロードできません!";
            }
            if(["png","jpg","jpeg","gif","bmp"].indexOf(exp) < 0 && size > config.size * 1024)
            {
                var errorstr = config.size+"KB 以上のファイルをアップロードできません!";
            } 
            if(errorstr)
            {
