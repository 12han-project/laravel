        ...
        this.each(function() {
            ...
            if (d != "" && h != true) {
                a("<span>").addClass("tag").append(a("<span>").text(d).append(" "),
                    a("<input>").addClass("tag").attr('type','checkbox').attr('name',f+'[]').val(d).attr("checked", true).css('display','none'),
                    a("<a>", {
                        href: "#",
                        title: "Removing tag",
                        text: "x"
            ...
            if (e.interactive) {
                        ...
                        function(b) {
                            ...
                            if (a(b.data.fake_input).val() != "" && a(b.data.fake_input).val() != c ) {
                                if (b.data.minChars <= a(b.data.fake_input).val().length && (!b.data.maxChars || b.data.maxChars >= a(b.data.fake_input).val().length)){
                                    var tag_val = a(b.data.fake_input).val();
                                    if(a(b.data.fake_input).val().indexOf('~')>0){
                                        var tags = a(b.data.fake_input).val().split('~');
                                        var start_tag = tags[0].toUpperCase();
                                        var end_tag = tags[1].toUpperCase();
                                        if(start_tag.substring(0,2) == end_tag.substring(0,2)){
                                            for (var i = start_tag.substring(2,7); i <= end_tag.substring(2,7); i++) {
                                                if (b.data.minChars <= a(b.data.fake_input).val().length && (!b.data.maxChars || b.data.maxChars >= a(b.data.fake_input).val().length)) a(b.data.real_input).addTag(start_tag.substring(0,2)+i, {
                                                    focus: true,
                                                    unique: e.unique
                                                })
                                            }
                                        }else{
                                            a(b.data.real_input).addTag(a(b.data.fake_input).val().toUpperCase(), {focus: true,unique: e.unique})
                                        }
                                    }else{
                                        var tag_val = a(b.data.fake_input).val();
                                        a(b.data.real_input).addTag(a(b.data.fake_input).val().toUpperCase(), {focus: true,unique: e.unique})
                                        if(tag_val == 'delet')$('div#'+d+'_tagsinput>span.tag').remove()
                                    }
                                }
                            }else {
                        ...
                        if (b.which == b.data.delimiter.charCodeAt(0) || b.which == 13) {
                            ...
                            if (b.data.minChars <= a(b.data.fake_input).val().length && (!b.data.maxChars || b.data.maxChars >= a(b.data.fake_input).val().length)){
                                if(a(b.data.fake_input).val().indexOf('~')>0){
                                    ...
                                    a(b.data.real_input).addTag(a(b.data.fake_input).val().toUpperCase(), {focus: true,unique: e.unique})
                                    if(tag_val == 'delet'){
                                        $('div#'+d+'_tagsinput>span.tag').remove();
                ...
                if (f.unique) {
                    a(f.fake_input).keydown(function(b) {
                        if (b.keyCode == 8 || String.fromCharCode(b.which).match(/\w+|[谩茅铆贸煤脕脡脥脫脷帽脩,/] + /)){a(this).removeClass("not_valid")}})}}});return this};a.fn.tagsInput.updateTagsField=function(c,d){var e=a(c).attr("id");a(c).val(d.join(b[e]))};a.fn.tagsInput.importTags=function(d,e){a(d).val("");var f=a(d).attr("id");var g=e.split(b[f]);for(i=0;i<g.length;i++){a(d).addTag(g[i].toUpperCase(),{focus:false,callback:false})}if(c[f]&&c[f]["onChange"]){var h=c[f]["onChange"];h.call(d,d,g[i])}}})(jQuery);
